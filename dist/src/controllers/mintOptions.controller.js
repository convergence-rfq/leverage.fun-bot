...
import { getEuroMetaPda, getExpirationDataPda, getOptionMintPda, getWriterMintPda, getUnderlyingPoolPda, getOptionCyclePda, getAuctionProgramVaultPda, getAuctionStatePda, getPoolAuthorityVaultsPda } from '../utils/pdas.js';
import { sleep, OptionTypeV2, postTelegramMessage, getAtaForUser } from '../utils/index.js';
import * as anchor from '@coral-xyz/anchor';
import BN from 'bn.js';
import { getProgram, getVaultsProgram, getAuctionProgram } from '../utils/programUtils.js';
import { TOKEN_PROGRAM_ID } from '@coral-xyz/anchor/dist/cjs/utils/token.js';
import { Transaction } from '@solana/web3.js';
import { getAssociatedTokenAddressSync } from '@solana/spl-token';
import cron from 'node-cron';
async function processMintingTransaction(provider, expiration, cycleNumber) {
    ...
    const [euroMetaKey, euroMetaBump] = getEuroMetaPda(Config.TESTNET_WETH_MINT, Config.TESTNET_QUOTE_MINT, expiration, OptionTypeV2.CALL, underlyingPool, strikePrices);
    ...
    const vaultUnderlyingAta = getAssociatedTokenAddressSync(euroMeta.underlyingMint, poolAuthorityVaults, true);
    ...
    const initAuctionIx = await auctionProgram.methods.initializeAuction(cycleNumber, auctionType, startingBid, duration).accounts({
...
export async function scheduleMintingProcess(provider) {
    ...
    cron.schedule('0 0 * * 5', async ()=>{
        ...
        try {
            ...
            const txHash = await processMintingTransaction(provider, expiration, cycleNumber);
            if (txHash) {
